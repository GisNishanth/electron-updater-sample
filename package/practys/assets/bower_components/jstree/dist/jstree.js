 ...
 */
	...
	var instance_counter = 0,
		...
		_d = document, _node = _d.createElement('LI'), _temp1, _temp2, _temp21, _temp22,_temp23,_temp24, _temp25, 
		_temp25a, _temp26, _temp26a, _temp27, _temp27a, _temp3,_temp4,_temp5, _temp5a, _temp2a, _temp2b,_temp2c,_temp2aa;
	...
	//_temp2.className = 'jstree-anchor-span pull-left';

	//_temp2 = _d.createElement('span');
	//_temp2.className = 'jstree-anchor-span';

	_temp2a = _d.createElement('div');
	_temp2a.className = 'jstree-anchor-span1 col-md-8';

	_temp2aa = _d.createElement('p');
	_temp2aa.className = 'main_user';

	_temp2b = _d.createElement('p');
	_temp2b.className = 'tree_user';

	_temp2c = _d.createElement('p');
	_temp2c.className = 'user_desin';
	
	_temp3 = _d.createElement('div');
	_temp3.className = 'popover-blk padding-zro plus_btn_ic';

	_temp5 = _d.createElement('div');
	_temp5.className = 'col-md-3 padding-zro';

	_temp5a = _d.createElement('img');
	_temp5a.className = 'img-circle';
	_temp5a.src = "assets/img/img2.jpg";

	_temp4 = _d.createElement('div');
	_temp4.className = 'pad-right';

	_temp21 = _d.createElement('ul');
	_temp21.className = 'pull-right';

	//_temp23 = _d.createElement('I');
	//_temp23.className = 'fa fa-ellipsis-v opentree-box pull-right edit_btn_ic';

	_temp22 = _d.createElement('li');

	_temp24 = _d.createElement('button');
	_temp24.innerHTML = 'Assign Task';

	_temp25 = _d.createElement('li');

	_temp25a = _d.createElement('button');
	_temp25a.innerHTML = 'Appraisal';


	_temp26 = _d.createElement('li');

	_temp26a = _d.createElement('button');
	_temp26a.innerHTML = 'Leave Sheet';

	_temp27 = _d.createElement('li');

	_temp27a = _d.createElement('button');
	_temp27a.innerHTML = 'View Profile';

	
	//_temp1.appendChild(_temp24);
	//_temp1.appendChild(_temp23);

	_temp1.appendChild(_temp3);

	_temp1.appendChild(_temp5);


	_temp3.appendChild(_temp4);

	_temp4.appendChild(_temp21);

	//_temp1.appendChild(_temp2);

	_temp21.appendChild(_temp22);
	_temp21.appendChild(_temp25);
	_temp21.appendChild(_temp26);
	_temp21.appendChild(_temp27);

	_temp22.appendChild(_temp24);
	_temp25.appendChild(_temp25a);
	_temp26.appendChild(_temp26a);
	_temp27.appendChild(_temp27a);

	_temp5.appendChild(_temp5a);
	_temp1.appendChild(_temp2a);
	
	_temp2a.appendChild(_temp2aa);
	_temp2a.appendChild(_temp2b);
	_temp2a.appendChild(_temp2c);
	
	_node.appendChild(_temp1);
	_temp1 = _temp2 = _temp21 = _temp22 = _temp23 = _temp24 = _temp25 = _temp25a = _temp26 = _temp26a = _temp27 = _temp27a = _temp3 = _temp4 = _temp5 = _temp5a = _temp2a = _temp2b = _temp2c = _temp2aa = null;
	...
	$.jstree.core.prototype = {
		...
		init : function (el, options) {
			...
			this._data.core.original_container_html
				...
				.remove();
			this.element.html("<"+"ul class='jstree-container-ul jstree-children' role='group'><"+"li id='j"+this._id+"_loading' class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='tree-item'><i class='jstree-icon jstree-ocl'></i><"+"a class='jstree-anchor' href='#'>" + this.get_string("Loading ...") + "<i class='jstree-icon jstree-themeicon-hidden'></i></a></li></ul>");
		...
		bind : function () {
			...
			this.element
				...
				.on("click.jstree", ".plus_btn_ic", $.proxy(function (e) { // kr added this line
						var ott = null;
						// $(".add_loc").hide();
						
						var ott = this.get_node(e.currentTarget);						
						var pr_o = this.get_parent(e.currentTarget);
						console.log(ott);
						console.log(pr_o);

						var input = $('#userId');
						//var forms = $('.taskForm');
						var submtBtn = $('#hSubmit');

						input.val(ott.id);	
						input.trigger('input');	

						setTimeout(function () { submtBtn.trigger('click'); }, 100);


					}, this))
				.on("click.jstree", ".edit_btn_ic", $.proxy(function (e) { // kr added this line
						/* var ott = null;
						$(".add_loc").hide();
						var ott = this.get_node(e.currentTarget);
						if(ott && ott.id && ott.id !== '#') {
							this.edit(ott);
						} */
						var ott = null;
						// $(".add_loc").hide();
						...
						var input = $('.parentStockingLocationId');
						var input1 = $('.loc_id');
						
						if(pr_o != '#'){
							// console.log($("#"+pr_o+"_anchor")[0].text.split('-'));
							input.val($("#"+pr_o+"_anchor")[0].text.split('-')[0]);							
							input1.val(pr_o);
						}else{
							input.val(ott.original.locationId);		
							input1.val('');
						}
						
						input.trigger('input');
						input1.trigger('input');
						var descrp = $('.stockingLocationDescription');
						descrp.val(ott.original.desc);
						descrp.text(ott.original.desc);
						descrp.trigger('input');
						
						var st_locid = $('.stockingLocationId');
						st_locid.val(ott.original.SlocationId);
						st_locid.trigger('input');
						st_locid.prop('readonly','readonly');
						
						var locaName = $('.locationName');
						locaName.val(ott.original.locationName);
						locaName.trigger('input');
						
						var input2 = $('._id');
						input2.val(ott.id);
						input2.trigger('input');
						
						var chkbox = $(".checkbox_chk");
						// if(ott.original.isactive){
						chkbox.prop('checked', ott.original.isactive);
						chkbox.trigger('input');
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			// kr added 
				node.childNodes[1].className += ' '+obj.original.Inactive;
			// ended
			...
			// karthick find  --  all nodes and their text

			if(this.settings.core.force_text) {
				node.childNodes[1].appendChild(d.createTextNode(obj.text));
			}
			else {
				//node.childNodes[1].childNodes[2].innerHTML += obj.text;
				node.childNodes[1].childNodes[2].childNodes[0].innerHTML += obj.text;

				node.childNodes[1].childNodes[2].childNodes[1].innerHTML += "userId# "+obj.original.empId;
				node.childNodes[1].childNodes[2].childNodes[2].innerHTML += obj.original.desc;
			}
		...
		refresh : function (skip_loading, forget_state) {
			...
			if(!skip_loading) {
				this.element.html("<"+"ul class='"+c+"' role='group'><"+"li class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='treeitem' id='j"+this._id+"_loading'><i class='jstree-icon jstree-ocl'></i><"+"a class='jstree-anchor' href='#'>" + this.get_string("Loading ...") + "<i class='jstree-icon jstree-themeicon-hidden'></i></a></li></ul>");
				this.element.attr('aria-activedescendant','j'+this._id+'_loading');
			}
		...
		rename_node : function (obj, val) {
			 ...
			 */
			var input = $('.parentStockingLocationId');
			input.val($("#"+obj.parent+"_anchor")[0].text);
			input.trigger('input');
			
			var input1 = $('.loc_id');
			input1.val(obj.parent);
			input1.trigger('input');
			
			var descrp = $('.stockingLocationDescription');
			descrp.val(obj.original.desc);
			descrp.text(obj.original.desc);
			descrp.trigger('input');
			
			var st_locid = $('.stockingLocationId');
			st_locid.val(obj.text);
			st_locid.trigger('input');
			
			var input2 = $('._id');
			input2.val(obj.id);
			input2.trigger('input');
			// $("form").submit(); // commented for checking
		...
		move_node : function (obj, par, pos, callback, is_loaded, skip_redraw) {
			 ...
			 */
			var input = $('.parentStockingLocationId');
			input.val(obj.parent);
			input.trigger('input');
			
			var input1 = $('.loc_id');
			input1.val(new_par.id);
			input1.trigger('input');
			...
			var st_locid = $('.stockingLocationId');
			st_locid.val(obj.original.SlocationId);
			st_locid.trigger('input');
			
			var st_locnme = $('.locationName');
			st_locnme.val(obj.original.locationName);
			st_locnme.trigger('input');
