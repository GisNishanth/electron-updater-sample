...
/***/ function(module, exports) {
	...
	module.exports = "<div\n  mwl-droppable\n  on-drop=\"vm.handleEventDrop(dropData.event, day.date, dropData.draggedFromDate)\"\n  mwl-drag-select=\"!!vm.onDateRangeSelect\"\n  on-drag-select-start=\"vm.onDragSelectStart(day)\"\n  on-drag-select-move=\"vm.onDragSelectMove(day)\"\n   on-drag-select-end=\"vm.onDragSelectEnd(day)\"\n  class=\"cal-month-day {{ day.cssClass }}\"\n  ng-class=\"{\n    'cal-day-outmonth': !day.inMonth,\n    'cal-day-inmonth': day.inMonth,\n    'cal-day-weekend': day.isWeekend,\n    'cal-day-past': day.isPast,\n    'cal-day-today': day.isToday,\n    'cal-day-future': day.isFuture,\n    'cal-day-selected': vm.dateRangeSelect && vm.dateRangeSelect.startDate <= day.date && day.date <= vm.dateRangeSelect.endDate\n  }\">\n\n   <span\n  ng-click=\"vm.toggleCalendar('month');vm.helpMe(day)\"\n   class=\"month-date\"\n    data-cal-date\n    ng-bind=\"day.label\">\n  </span>\n\n  <div class=\"cal-day-tick\" ng-show=\"dayIndex === vm.openDayIndex && vm.view[vm.openDayIndex].events.length > 0 && !vm.slideBoxDisabled\">\n    <i class=\"glyphicon glyphicon-chevron-up\"></i>\n    <i class=\"fa fa-chevron-up\"></i>\n  </div>\n\n  <ng-include src=\"vm.customTemplateUrls.calendarMonthCellEvents || vm.calendarConfig.templates.calendarMonthCellEvents\"></ng-include>\n\n <div  ng-repeat=\"(y,x) in day.appoCount \" > <div ng-if=\"value.clinicId == day.clinicId && y < 3\"   ng-repeat=\"(key,value) in x.colorCode \"><small\n    class=\"cal-events-num badge badge-important event-count pull-left\"\n  ng-style=\"{backgroundColor: value.colorCode}\"\n   ng-show=\"day.badgeTotal > 0\"\n    ng-bind=\"x.appCount\">\n  </small>\n\n   </div>\n\n  </div>\n <p ng-bind=\"\">\n </p>\n <a rel=\"popover\" ng-click=\"vm.popOver($event)\" ng-if=\"day.appoCount.length > 3\" class=\"calendar-popover\"   >\n <i data-toggle=\"popover\" class='fa fa-angle-double-down' aria-hidden='true' data-id={{day.label}} ></i> </a>\n <div hidden id=\"pop-over-{{day.label}}\" > <div   ng-repeat='(y,x) in day.appoCount ' > <div ng-if='value.clinicId == day.clinicId'   ng-repeat='(key,value) in x.colorCode '><small class='cal-events-num badge badge-important event-count pull-left' ng-style='{backgroundColor: value.colorCode}'   ng-show='day.badgeTotal > 0'  ng-bind='x.appCount'> </small> </div> </div> </div>\n";
	...
	angular
	  ...
	  .controller('MwlCalendarCtrl', ["$scope", "$log", "$timeout", "$attrs", "$locale", "moment", "calendarTitle", "calendarHelper", function($scope, $log, $timeout, $attrs, $locale, moment, calendarTitle, calendarHelper) {
	    ...
	    vm.changeView = function(view, newDay) {
	      ...
	      console.log(vm.view,"viewwwwwwwwwwwwwwwwwwwww");
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getMonthView = function (_a) {
		// console.log(_a,"exports 1111111");
	    var events = [], viewDate = _a.viewDate;
	    for(var i=0; i<_a.events.length;i++){
	    	if(_a.events[i].status != 'block'){
	    		events.push(_a.events[i]);
	    	}
	    	
	    }
	    // console.log(events,"exports 111111111111111111111");
	    var start = moment(viewDate).startOf('month').startOf('week');
	    var end = moment(viewDate).endOf('month').endOf('week');
	    // console.log(events,"exports evemntzzzzzzzzzzz");
	    // console.log(start,"exports startttttttttttttt");
	    // console.log(end,"exports enddddddddddd");
	    ...
	    // console.log(eventsInMonth,"eventzzzzzzzzzzz in monthhhhhhhhhhhhhhh");
	    var days = [];
	    var doctorAppointmentCount = [];
	    for (var i = 0; i < end.diff(start, 'days') + 1; i++) { //end.diff(start, 'days') =>count of days in the view of month
	        var date = start.clone().add(i, 'days');
	        ...
	        // console.log(events_1,"eventsssssssssssss");
	        var doctorAppointmentCountArr = [];
	        var z = [];
	        if(events_1.length > 0){
	        	var g = _.groupBy(events_1, 'description');
	        	var vals = Object.keys(g).map(function(key) {
				    return g[key];
				});	
				// console.log(vals,"valueeeeeeeeeeeeee...........");			
	        	
	        	for(var j =0 ; j< vals.length ; j++){
	        		// for(var k = 0; k < vals[i].length ; k++){
	        			var clinicId = vals[j][0].clinicId;
	        			z.push({'appCount':vals[j].length,'colorCode':vals[j][0].colorCode})
	        		// }
	        	}
	        	
	        }
	   		day.clinicId = clinicId;
	        day.appoCount = z;
	        day.inMonth = date.clone().startOf('month').isSame(moment(viewDate).startOf('month'));
	        day.events = events_1;
	        day.badgeTotal = events_1.length;
	        days.push(day);
	        // console.log(day.appoCount,"apooooooooooooooo");
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	angular
	  ...
	  .controller('MwlCalendarMonthCtrl', ["$scope", "moment", "calendarHelper", "calendarConfig", "calendarEventTitle", function($scope, moment, calendarHelper, calendarConfig, calendarEventTitle) {
	    ...
	    vm.highlightEvent = function(event, shouldAddClass) {
	      ...
	      vm.helpMe = function(event, newDayDate, draggedFromDate ) {
	    	if(event.isToday == true || event.isFuture == true){
	    		// console.log(moment(event.date._d).format("YYYY-MM-DD"));
	    		var date = moment(event.date._d).format("YYYY-MM-DD");
	    		$scope.$parent.$parent.$parent.$parent.vm.calendarView = 'day';
	    		$scope.$parent.$parent.$parent.$parent.vm.toggleCalendar('day',null,date);
	    	// 	console.log($scope.$parent.$parent.$parent.$parent);
	    	// console.log(event);
	    	// console.log(newDayDate);
	    	// console.log(draggedFromDate);
	    }else{
	    	console.log("cant load it is past");
	    	return false;
	    }
	};
	...
	vm.popOver = function(e){
		...
		var content = $('#pop-over-'+e.target.dataset.id).html(); 

		$(e.target).popover({placement: "bottom",html: true, content: function () { return content; } }).popover('show'); 
...
/***/ function(module, exports, __webpack_require__) {
	...
	angular
	  ...
	  .controller('MwlCalendarWeekCtrl', ["$scope", "moment", "calendarHelper", "calendarConfig", "calendarEventTitle", function($scope, moment, calendarHelper, calendarConfig, calendarEventTitle) {
		...
		// kr added this feature
	    vm.calcWeekHeight = function(event) {
	     	var startDate = moment(event.startsAt).format("YYYY-MM-DD hh:mm");
            var dbEndDate = moment(event.endsAt).format("YYYY-MM-DD hh:mm");

            var nowEventDate = moment(startDate); //todays date
            var endEventDate = moment(dbEndDate); // another date

            var remHours = endEventDate.diff(nowEventDate, 'minutes');
	...
	angular
	  ...
	  .filter('calendarTruncateEventTitle', function() {
	    ...
	    return function(string, length, boxHeight) {
	      ...
	      if (string.length >= length && string.length / 20 > boxHeight / 50) {
	...
	angular
	  ...
	  .constant('calendarConfig', {
	    ...
	    colorTypes: {
	      info: {
	        primary: '#53D1D0',
	        secondary: '#53D1D0' // future dates
	      },
	      important: {
	        primary: '#8CA2EB',
	        secondary: '#8CA2EB' // ongoing dates // current
	      },
	      warning: {
	        primary: '#F0F4F7',
	        secondary: '#F0F4F7'  // past dates
	      },
	      inverse: {
	        primary: '#1b1b1b',
	        secondary: '#c1c1c1'
	...
	angular
	  ...
	  .factory('calendarHelper', ["$q", "$templateRequest", "dateFilter", "moment", "calendarConfig", function($q, $templateRequest, dateFilter, moment, calendarConfig) {
	    ...
	    function getMonthView(events, viewDate, cellModifier) {
	    	// console.log(events,"eventsssssssssssssssssssssss");
	    	console.log(viewDate,"vieww Dateeeeeeeeeee");
	      ...
	      console.log(events,"eventssssssssssssss");
	      ...
	      view.days = view.days.map(function(day) {
	        ...
	        cellModifier({calendarCell: day});
	         // console.log(cellModifier({calendarCell: day}),"day viewwwwwwwwwwwwwws");
	        return day;
	      });
	      ...
	      console.log(view.days[0].date._d,"eventssssssssssssssssssssss viewwwwwwwwwwwwwws");
	      return view;
	  ...
	  .factory('calendarTitle', ["moment", "calendarConfig", "calendarHelper", function(moment, calendarConfig, calendarHelper) {
	    ...
	    function week(viewDate) {
	      return calendarConfig.titleFormats.week
	      	.replace('{MM}', moment(viewDate).startOf('week').format('MMMM YYYY'))
